...
export function BanksContent() {
  const { banks, isLoading, isError, createBank, updateBank, deleteBank } = useBanks();
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 w-full p-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-muted-foreground">Manage your financial institutions and view consolidated summaries</p>
        <Button onClick={handleAddBank} className="h-11 px-6 py-2 gap-2 transition-all hover:scale-105 active:scale-95 rounded-xl font-bold">
      ...
      <QueryBoundary 
        ...
        loadingComponent={
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div key={i} className="h-[320px] w-full rounded-3xl animate-pulse bg-muted/50 border" />
            ))}
          </div>
        }
      >
        {!banks || banks.length === 0 ? (
          <div className="flex min-h-[400px] flex-col items-center justify-center rounded-3xl border-2 border-dashed bg-muted/20 p-12 text-center animate-in zoom-in-95 duration-300">
            <div className="flex h-24 w-24 items-center justify-center rounded-3xl bg-muted border mb-6">
              <Building2 className="h-12 w-12 text-muted-foreground/50" />
            </div>
            <h2 className="text-2xl font-bold mb-2">No banks found</h2>
            <p className="max-w-[340px] text-muted-foreground mb-8">
