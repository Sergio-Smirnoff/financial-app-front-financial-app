...
import { useLoans, useLoanInstallments, usePayLoanInstallment, useDeleteLoan } from '@/lib/hooks/useLoans'
...
interface Props { bankNumber?: string }

export function LoanList({ bankNumber }: Props) {
  const { data: loans, isLoading, isError } = useLoans(bankNumber)
  ...
  const handleDelete = (loan: Loan) => {
    openConfirmDelete({
      ...
      onConfirm: () => deleteLoan.mutate(loan.id, {
        ...
        onError: (e) => toast.error(e.message || 'Failed to delete loan'),
      }),
    })
  }
            ...
            {inst.paid ? (
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  onClick={() => payInstallment.mutate(
                    { loanId, installmentId: inst.id, accountCbu: selectedAccounts[inst.id]! },
                    {
